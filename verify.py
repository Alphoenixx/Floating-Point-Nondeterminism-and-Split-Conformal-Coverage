...
def main():
    ...
    all_pass = bool(a.get("all_passed", False)) and bool(b.get("all_passed", False)) and stress_ok and flips_ok
    eps = syn.get("eps_max", float("nan"))
    print("ALL CERTIFICATE CHECKS {} ({}/{} alpha synthetic + {}/{} alpha real, stress {}) | top-1 flips = {} | eps ~ {:.0e} (fp16)".format(
        "PASSED" if all_pass else "NOT PASSED", syn_a, syn_t, real_a, real_t,
        "ok" if stress_ok else "FAIL", "0" if flips_ok else "NONZERO", eps))
