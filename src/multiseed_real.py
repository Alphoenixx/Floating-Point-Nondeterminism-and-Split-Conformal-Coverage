...
per = {a: {"gap": [], "churn": [], "cert": [], "within": [], "cov_calib": [], "cov_deploy": []} for a in ALPHAS}
...
for s in range(N_SEEDS):
    ...
    for a in ALPHAS:
        ...
        per[a]["cov_calib"].append(cA)
        per[a]["cov_deploy"].append(cB)
...
for a in ALPHAS:
    ...
    results["per_alpha"][str(a)] = {
        ...
        "cov_calib": agg(per[a]["cov_calib"]),
        "cov_deploy": agg(per[a]["cov_deploy"]),
